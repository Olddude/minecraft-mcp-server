...
/**
 * Copies the LICENSE file from the current working directory to the 'dist' directory.
 * This is useful for including the license information in the distribution package,
 * ensuring that users of the package are aware of the licensing terms.
 */
function copyLicense() {
    const workDir = cwd();
    const serverOutputDirectory = join(workDir, 'dist');
    copyFileSync(join(workDir, 'LICENSE'), join(serverOutputDirectory, 'LICENSE'));
    console.debug('Copied LICENSE to', serverOutputDirectory);
}
